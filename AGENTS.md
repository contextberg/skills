...
This file connects your agent to the Contextberg MCP server to retrieve and update information that Contextberg has collected and generated on the user's local machine. Supported sources for agent transcripts: **Claude Code, Cursor, OpenClaw**.
...
The Contextberg desktop app must be running. Configure the MCP server in your agent. Example for Claude Code (`.claude/settings.local.json`):
...
For other MCP-aware agents (Cursor, OpenClaw, Windsurf, Cline, Continue, etc.), point them at the same `npx -y @contextberg/mcp-server` command via their MCP configuration.
